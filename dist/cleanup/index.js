 ...
 */
export async function deleteWorker(workerName, apiToken, accountId) {
 ...
 */
export function buildWorkerList(options) {
...
export async function cleanup(options) {
    ...
    if (options.failOnError && errorCount > 0) {
        throw new Error(`Some operations failed. Deleted: ${deletedCount}, Not found: ${notFoundCount}, Errors: ${errorCount}`);
    }
    return {
        deletedCount,
        notFoundCount,
