...
- d5dbabd: Fix preview Worker naming and improve deployment action behavior.
...
- fd62f86: Fix `deleteWorker`/`getWorker` to detect "not found" via HTTP status (404) instead of matching error message text, so Cloudflare's `This Worker does not exist on your account.` response is correctly treated as a skip rather than a failure.
